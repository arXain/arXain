...
<div>
    <form>
        <div class="form-group">
            <button type="submit" class="btn btn-primary btn-submit" :disabled="contractRevised" v-on:click.prevent="amendIpfs(fileDir)">Submit Paper</button>
        </div>
        <p><span v-html="messageAmend"></span></p>
        <div class="form-group">
           <label for="contract">Revised Paper Hash:</label>
            <div class="input-group mb-3">
                <input type="text" :disabled="true" class="form-control" placeholder="Submit the contract to get the Revised Paper Hash" v-model="paperHash">
...
<script>
import {  submitRevision } from '../js/ipfs-helper.js'
...
export default {
    ...
    methods: {
		...
		this.localWeb3.eth.getAccounts(function(error, accounts) {
			...
			el.$emit("ipfs:message", "Uploading paper revision to IPFS...");
			submitRevision(account, contract, paperDir).then(function (response) {
		...
		amendContract: function(el, account) {
            ...
            el.messageAmend = 'Revising the contract...';
            console.log('paper hash is '+paperHash);
            paperInstance.amendPaper(paperHash).then(function(result) {
                el.messageAmend = 'Success! Your paper has been revised.';
                el.contractRevised = true;
                console.log('Successful submission');
            }).catch(function(err) {
                el.messageAmend = 'paper revision failed!';
